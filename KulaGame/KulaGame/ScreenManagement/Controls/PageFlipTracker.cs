...
namespace KulaGame.ScreenManagement.Controls
{
    ...
    {
        public const GestureType GesturesNeeded = GestureType.Flick | GestureType.HorizontalDrag | GestureType.DragComplete | GestureType.Tap;
        ...
        public bool HighscoresWasTapped
        {
            get;
            set;
        }
            ...
            foreach (GestureSample sample in input.Gestures)
            {
                switch (sample.GestureType)
                {
                    ...
                    case GestureType.Tap:
                        ...
                        if (sample.Position.X > 540 && sample.Position.X < 800 && sample.Position.Y > 420 && sample.Position.Y < 480)
                        {
                            HighscoresWasTapped = true;
                        }
                        else
                        {
                            WasTapped = true;
